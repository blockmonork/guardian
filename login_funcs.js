 ...
 * 
 */
var msgLen = `must be between ${_v_[0]} and ${_v_[1]} characters`;
const _d = document;

function sbmt() {
    if (checkLogin()) {
        _g(_v_[2]).value = _s(_v_[2]);
        _g(_v_[3]).value = _s(_v_[3]);
...
function _s(i) {
    return CryptoJS.SHA256(_g(i).value);
}
...
function checkLogin() {
    var e = _g(_v_[2]).value,
        t = _g(_v_[3]).value,
        s = !0;
    return e.length < _v_[0] || e.length > _v_[1] ? (setHelper(_v_[2], `user ${msgLen}`), s = !1) : setHelper(_v_[2], "right"), t.length < _v_[0] || t.length > _v_[1] ? (setHelper(_v_[3], `pass ${msgLen}`), s = !1) : setHelper(_v_[3], "right"), !!s && checkPass()
}
