...
ini_set('php_errors', 1);
ini_set('display_errors', 1);
ini_set('display_startup_errors', 1);
ini_set('memory_limit', '128M');
error_reporting(E_ALL);
ini_set('php_error_log', 'errospeagape');
session_start();
if ( !isset($_SESSION['ABFT']) ) {
    $_SESSION['ABFT'] = time();
    echo "session start now at " . $_SESSION['ABFT'] . "<br>";
};

$abft = ( isset($_SESSION['ABFT']) ) ? $_SESSION['ABFT'] : 0;
echo "Session ".$abft."<br>";
//echo PHP_SESSION_ACTIVE."<br>";
...
class Guardian
{
    ...
    private static $FORM_USERNAME = 'f1'; // random generated if empty
    private static $FORM_PASSWORD = 'f2'; // random generated if empty
    ...
    // public function that prints header Information
    /**
     * 
     * @return void
     */
    ...
    {
        //exit("parei no printHeader...antes da saida");
        /*
    ...
    /**
     * @return string the main page of the site
     */
    ...
    /**
     * @return string the logged page to redirect to after login
     */
    ...
    /**
     * @return array the form fields for username and password
     */
    public static function getFormFields()
    {
        if ( self::$FORM_USERNAME == '' ){
            self::$FORM_USERNAME = self::getFormName();
        }
        if ( self::$FORM_PASSWORD == ''){
            usleep(100);
            self::$FORM_PASSWORD = self::getFormName();
        }
    ...
    /**
     * @return int the maxlength for username and password fields
     */
    ...
    /**
     * @return int the banishement time in seconds
     */
    ...
    {
        self::$HARD_USER = md5($user);
        self::$HARD_PASS = md5($pass);
    ...
    // generates the form name with ALPHANUMERIC 6 characters long
    /**
    ...
    public static function getFormName()
    {
        $name = '';
        for ($i = 0; $i < 10; $i++) {
            $name .= self::$ALPHANUMERIC[rand(0, strlen(self::$ALPHANUMERIC) - 1)];
        }
        return $name;
    }
    ...
    public static function initCredentials()
    {
        ...
        // apply md5 to the password and the user variables
        self::$PASS = md5(self::$PASS);
        self::$USER = md5(self::$USER);
    ...
    {
        $k = strtoupper($k);
        return (isset($_SESSION[$k]) && !empty($_SESSION[$k])) ? $_SESSION[$k] : false;
    }
    ...
    {
        //$val = substr($val, 0, self::$MAX_LENGTH); No More! values comes MD5ed now.
        $unwanted_chars = [
            '"',
            '<',
            '>',
    ...
    /**
     * get brute force time from session variable
     * @param string $autoCreateSession if true, create session if it does not exist (default: false)
     * @return int the time in seconds
     */
    private static function getAntiBruteForceTime($autoCreateSession = false)
    {
        $key = 'ABFT';
        $time = self::getSessVar($key);
        if (($time == '' || !$time) && $autoCreateSession) {
    ...
    {
        $abft = self::getAntiBruteForceTime(true);
        echo "isBruteForce abft $abft <br>"; exit;
        return (time() < $abft) ? true : false;
    }
